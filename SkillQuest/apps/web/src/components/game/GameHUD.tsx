...
const COMBO_TIER_STYLES: Record<string, string> = {
  good: 'text-yellow-400 border-yellow-500/30 bg-yellow-500/5',
  great: 'text-orange-400 border-orange-500/30 bg-orange-500/5',
  amazing: 'text-red-400 border-red-500/30 bg-red-500/5',
  legendary: 'text-purple-400 border-purple-500/30 bg-purple-500/5 animate-pulse',
};
    ...
    <div className="pointer-events-none absolute inset-x-0 top-0 z-10 p-3">
      ...
      <div className="pointer-events-auto glass flex items-center justify-between rounded-xl px-4 py-2.5">
        ...
        <div className="flex items-center gap-3">
          {levelTitle && (
            <span className="text-sm font-medium text-base-100 truncate max-w-[200px]">
              {levelTitle}
            </span>
          )}
        ...
        <div className="flex items-center gap-4 text-sm">
          ...
          <span className="flex items-center gap-0.5 text-base-300">
            {Array.from({ length: 3 }, (_, i) => (
              <Star
                key={i}
                size={14}
                strokeWidth={1.5}
                className={i < stars ? 'fill-yellow-400 text-yellow-400' : 'text-base-600'}
              />
            ))}
          </span>
          {remainingSec !== null && (
            <span className={`flex items-center gap-1 font-mono text-xs ${remainingSec < 10 ? 'text-red-400 animate-pulse' : 'text-base-400'}`}>
      ...
      {/* Progress bar */}
      <div className="mt-2 h-[2px] rounded-full bg-base-700/60 overflow-hidden">
        <div
