...
fn test_entity_count_stable_over_ticks() {
    ...
    assert_eq!(
        core.entity_count(),
        before,
        "no entities should appear or disappear"
    );
}
...
fn test_reject_invalid_cpu() {
    ...
    assert!(
        SimulationCore::from_opdl(bad).is_err(),
        "cpu=1.5 must fail validation"
    );
}
...
fn test_all_entity_types_accepted() {
    for et in &["Server", "Switch", "Storage", "VM"] {
        let json = format!(
            r#"{{"version":"1.0","pack_id":"t",
            "entities":[{{"id":"x","entity_type":"{et}",
            "components":{{"cpu":0.1,"memory":0.1}}}}]}}"#
        );
        assert!(
            SimulationCore::from_opdl(&json).is_ok(),
            "{et} must be accepted"
        );
    }
}
