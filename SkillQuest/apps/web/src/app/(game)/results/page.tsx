    ...
    <div className="min-h-screen bg-surface flex items-center justify-center px-4 py-10">
      <div className="w-full max-w-lg">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="mt-4 text-2xl font-semibold text-base-900">
            {passed ? '恭喜完成挑战' : '再接再厉'}
          </h1>
          ...
          {[1, 2, 3].map((s) => (
            ...
            >
              <Star
                size={40}
                strokeWidth={1.5}
                className={s <= result.stars ? 'text-amber-600 fill-yellow-400' : 'text-base-600'}
              />
            </div>
          ))}
        </div>
        ...
        <div className="rounded-2xl border border-base-200 bg-white p-6 mb-6">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-4xl font-bold text-base-900">
          ...
          <div className="grid grid-cols-3 gap-4 text-center">
            <div className="rounded-xl bg-base-100 p-3">
              <Target size={16} strokeWidth={1.5} className="mx-auto text-blue-600 mb-1" />
              <p className="text-lg font-semibold text-base-900">{result.baseScore}</p>
              <p className="text-xs text-base-400">Base Score</p>
            </div>
            <div className="rounded-xl bg-base-100 p-3">
              <Clock size={16} strokeWidth={1.5} className="mx-auto text-emerald-600 mb-1" />
              <p className="text-lg font-semibold text-base-900">+{result.timeBonus}</p>
              <p className="text-xs text-base-400">Time Bonus</p>
            </div>
            <div className="rounded-xl bg-base-100 p-3">
              <Zap size={16} strokeWidth={1.5} className="mx-auto text-orange-600 mb-1" />
              <p className="text-lg font-semibold text-base-900">+{result.comboBonus}</p>
          ...
          <div className="mt-6 grid grid-cols-3 gap-4 text-center text-sm">
            <div>
              <p className="text-base-900 font-medium">{correctCount}/{totalCount}</p>
              <p className="text-xs text-base-400">Correct</p>
            </div>
            <div>
              <p className="text-base-900 font-medium">{maxCombo}x</p>
              <p className="text-xs text-base-400">Max Combo</p>
            </div>
            <div>
              <p className="text-base-900 font-medium">+{result.xpGained} XP</p>
        ...
        {result.achievements.length > 0 && (
          <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 mb-6">
            <h3 className="text-sm font-medium text-amber-600 mb-3 flex items-center gap-1.5">
              <Trophy size={14} strokeWidth={1.5} />
              Achievements Unlocked
            </h3>
            <div className="space-y-2">
              {result.achievements.map((a) => (
                <div key={a.id} className="flex items-center gap-3 rounded-lg bg-white px-3 py-2">
                  <span className="text-lg">{a.icon}</span>
                  <div>
                    <p className="text-sm font-medium text-base-900">{a.name}</p>
        ...
        <div className="flex gap-3">
          <Link
            ...
            className="flex-1 flex items-center justify-center gap-1.5 rounded-xl border border-base-200 bg-white px-4 py-3 text-sm text-base-600 transition hover:border-accent/40 hover:text-base-900"
          >
          ...
          <Link
            ...
            className="flex items-center justify-center gap-1.5 rounded-xl border border-base-200 bg-white px-4 py-3 text-sm text-base-600 transition hover:border-accent/40 hover:text-base-900"
          >
